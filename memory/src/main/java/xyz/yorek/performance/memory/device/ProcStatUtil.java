...
public final class ProcStatUtil {
    private static final String TAG = "ProcStatUtil";
    ...
    public static ProcStat parse(String path) {
        try {
            ...
            if (procStatInfo == null || procStatInfo.comm == null) {
                Log.w(TAG, "#parseJiffies read with buffer fail, fallback with spilts");
                try {
                    procStatInfo = parseWithSplits(LinuxVNodeUtils.cat(path));
                } catch (ParseException e) {
                    if (sParseError != null) {
                ...
                if (procStatInfo == null || procStatInfo.comm == null) {
                    Log.w(TAG, "#parseJiffies read with splits fail");
                    return null;
                }
            }
            return procStatInfo;
        } catch (Throwable e) {
            Log.w(TAG, "#parseJiffies fail: " + e.getMessage());
            if (sParseError != null) {
                sParseError.onError(0, LinuxVNodeUtils.cat(path) + "\n" + e.getMessage());
            }
            return null;
        }
    }
    ...
    public static ProcStat parseWithBufferForPath(String path, byte[] buffer) throws ParseException {
        ...
        try {
            ...
            Log.e(TAG, "read buffer from file fail" + Log.getStackTraceString(e));
            readBytes = -1;
        }
        if (readBytes <= 0) {
            return null;
        ...
        for (int i = 0, spaceIdx = 0; i < statBytes;) {
            ...
            switch (spaceIdx) {
                ...
                case 14: { // utime
                    ...
                    stat.utime = LinuxVNodeUtils.parseLong(num, 0);
                    break;
                }
                case 15: { // stime
                    int readIdx = i, window = 0;
                    ...
                    stat.stime = LinuxVNodeUtils.parseLong(num, 0);
                    break;
                }
                case 16: { // cutime
                    int readIdx = i, window = 0;
                    ...
                    stat.cutime = LinuxVNodeUtils.parseLong(num, 0);
                    break;
                }
                case 17: { // cstime
                    int readIdx = i, window = 0;
                    ...
                    stat.cstime = LinuxVNodeUtils.parseLong(num, 0);
                    break;
                }
    ...
    static ProcStat parseWithSplits(String cat) throws ParseException {
        ...
        if (!TextUtils.isEmpty(cat)) {
            ...
            stat.stat = splits[1];
            stat.utime = LinuxVNodeUtils.parseLong(splits[12], 0);
            stat.stime = LinuxVNodeUtils.parseLong(splits[13], 0);
            stat.cutime = LinuxVNodeUtils.parseLong(splits[14], 0);
            stat.cstime = LinuxVNodeUtils.parseLong(splits[15], 0);
        }
        return stat;
    }
    ...
    static String safeBytesToString(byte[] buffer, int offset, int length) {
        try {
            CharBuffer charBuffer = Charset.forName("UTF-8").decode(ByteBuffer.wrap(buffer, offset, length));
            return String.valueOf(charBuffer.array(), 0, charBuffer.limit());
        } catch (IndexOutOfBoundsException e) {
            Log.w(TAG, "#safeBytesToString failed: " + e.getMessage());
            return "";
        }
    }
